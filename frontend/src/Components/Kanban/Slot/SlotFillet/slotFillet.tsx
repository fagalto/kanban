...
const SlotFillet = (slot: slotData) => {
  const req_capacity = typeof slot.req_capacity === "number" ? slot.req_capacity : 0;
  const slotPercentFill =
    typeof slot.balance === "number" && req_capacity > 0
      ? (100 * slot.balance) / req_capacity
      : 0;
  const slotFillColor: string =
    slotPercentFill > 80 ? "rgb(113, 183, 67)" : "rgb(255, 153, 0)";
  const slotBackgroundFillet: CSS.Properties = {
    backgroundColor:
      slot.itemid != undefined && slot.itemid.length > 2 && slot.balance == 0
        ? "#F5447D"
        : "",
  };
  ...
  if (
    ((typeof slot.balance !== "number" &&
      slot.balance !== undefined) ||
